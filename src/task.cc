...
void WorkLogEntry::set_id(std::string id) {
    this->id = id;
}
...
void WorkLogEntry::set_done(int done) {
    this->done = done;
}
...
std::string WorkLogEntry::get_id() {
    return this->id;
}

std::string WorkLogEntry::get_task_id() {
    return this->task_id;
}

std::string WorkLogEntry::get_task_name() {
...
int WorkLogEntry::get_start_hour_entry() {
    return this->start_hour_entry;
}

int WorkLogEntry::get_done() {
    return this->done;
}

std::string WorkLogEntry::get_end_date_entry() {
    return this->end_date_entry;
}

int WorkLogEntry::get_end_hour_entry() {
    return this->end_hour_entry;
}
...
Task* Task::first(std::string task_id) {
    char sql[SQL_SIZE];
    sprintf(sql, SELECT_TASK_BY_ID, task_id.c_str());

    tasks_aux.clear();
    execute_query(sql, load_task);

    return tasks_aux.front();
}
...
static int load_worklog_entry(void* NotUsed, int argc, char** argv,
    ...
    Task* task = Task::first(task_id);

    WorkLogEntry* entry = new WorkLogEntry(
        task_id, argv[2] ? argv[2] : "", argv[3] ? atoi(argv[3]) : 0,
        argv[4] ? argv[4] : "", argv[5] ? atoi(argv[5]) : 0, task->get_done());
